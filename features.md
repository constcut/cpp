...
Структура, располагаемая в ОП так, как её описал программист.
...
P.s.: Функция fork создаёт новый процесс, а не поток, со своим адресным пространством.
...
+ Есть сложная функция, которая осуществляет вычисления, а после сохраняет их в файл. Ее необходимо разделить на две: вычисления и сохранения. Это позволит тестировать фукнцию вычислений.
...
Класс может быть унаследован, но его изначальный функционал не должен быть изменен, за исключением исправления ошибок. 

В качестве класса родителя могут выступать как абстрактные, так и обычные классы. В первом случае сохраняется интерфейс родительского класса, во втором функционал.

Паттерны связанные с принципом открытости/закрытости:
...
Данный принцип может быть нарушен в паттерне Шаблонный метод.

### Принцип разделения интерфейса

Много интерфейсов, специально предназначенных для клиентов, лучше, чем один интерфейс общего назначения.

Т.е. интерфейс решающий одну задачу, не должен знать ничего об интерфейсе, не связанном с этой задачей.
...
Пример тесно связанных модулей:

![image info](images/dependency.png)

Развязывание зависимостей, при помощи интерфейсов:

![image info](images/dep_solution.png)

Еще один пример тесной связанности:

Пример тесно связанных модулей:

![image info](images/dependency_2.png)

Развязывание зависимостей, при помощи интерфейсов:

![image info](images/dep_solution_2.png)

И еще один пример:

![image info](images/dep_inv.png)
